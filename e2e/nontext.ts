 ...
 *
 * On this page the first of those is not hypothetical. Every button in the lab
 * is `.btn { background: var(--surface-2); border: 1px solid var(--border) }`
 * sitting on a `.panel { background: var(--surface) }`, and `--border` is the
 * palette's SURFACE-DIVIDER token — the same one that draws the line between a
 * panel and the page. Measured through this oracle, the eleven buttons'
 * strongest delineator came to 1.65:1 in both themes against a 3:1 floor, and
 * nothing in the gate this replaces looked at a button boundary at all.
 *
 ...
 *
 *    The native-widget exemption below is load-bearing here: this lab's theft
 *    planner and break-it grid are fifty `<input type="checkbox">` elements
 *    left entirely to the user agent apart from an `accent-color`, and WCAG
 *    1.4.11 exempts a component whose appearance the author has not taken over.
 *
 ...
 * background beneath — because this page's verdict chips, its alarm rows and
 * its hero aside are all `color-mix()` over a panel, and reading the declared
 * value would measure something the page never paints.
 */
...
export async function auditNonText(page: Page): Promise<NonTextFailure[]> {
  return page.evaluate(() => {
    ...
    // oracles cannot drift into disagreeing about what a pixel is. Not every
    // helper in it is reached from this file's two checks; `noUnusedLocals`
    // would reject them, and deleting them would fork the core. Referencing
    // them is the cheapest way to keep one copy.
    void splitTopLevel;
    void over;
    void fade;
    void luminance;
